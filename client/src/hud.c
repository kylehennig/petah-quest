...
void draw_weapon_sel(uint8_t weapon) {
  start_color();
    ...
    mvaddch(LINES,MAX_HEALTH,' ');
    for(int i = 0; i < NUM_WEAPONS; i++){
      mvaddch(LINES,i+MAX_HEALTH+1,weapons[i]);
    }
    mvaddch(LINES,MAX_HEALTH+1+NUM_WEAPONS,' ');
    attroff(COLOR_PAIR(10));

    //load highlighted
    attron(COLOR_PAIR(11));
    mvaddch(LINES,weapon+MAX_HEALTH+1,weapons[weapon]);
    attroff(COLOR_PAIR(10));
}
...
void show_text(char *txt){
  ...
  mvprintw(LINES,NUM_WEAPONS+MAX_HEALTH+padded,txt);
